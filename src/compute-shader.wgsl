...
[[group(0), binding(0)]]
var input_list: [[access(read)]] InputBuffer;
[[group(0), binding(1)]]
var output_list: [[access(read_write)]] ResultBuffer;
...
[[group(1), binding(3)]]
var<storage> config_data: [[access(read_write)]] ConfigData;
...
fn rand_float_generate() -> f32 {
    let state: u32 = config_data.seed;
    config_data.seed = state * 747796405u + 2891336453u;
...
fn generate_scatter_ray_dir(hit_info: HitInfo) -> ScatterResult {
    if (hit_info.hit_material < 10) {
        var result: ScatterResult;

        let a: f32 = rand_float_generate();

        let b: f32 = rand_float_generate();
...
fn light_get_direct_shading_data(panel_point0: vec4<f32>, panel_point1: vec4<f32>, panel_normal: vec4<f32>) -> LightShadingData {
    var pdf_mul: f32 = -1.0;
    ...
    if (abs(panel_normal.x) > 0.0) {
        ...
        let y_sample = rand_float_generate() * y_width + panel_point0.y;
        let z_sample = rand_float_generate() * z_width + panel_point0.z;
    ...
    if (abs(panel_normal.y) > 0.0) {
        ...
        let x_sample = rand_float_generate() * x_width + panel_point0.x;
        let z_sample = rand_float_generate() * z_width + panel_point0.z;
    ...
    if (abs(panel_normal.z) > 0.0) {
        ...
        let y_sample = rand_float_generate() * y_width + panel_point0.y;
        let x_sample = rand_float_generate() * x_width + panel_point0.z;
...
struct HitInfoArrayData {
    data: [[stride(80)]] array<PointIntersectData, 20>;
};

fn generate_hit_info_array(ray_in_origin: vec4<f32>, ray_in_direction: vec4<f32>) -> HitInfoArrayData {
    ...
    loop {
        ...
        let sample_data = light_get_direct_shading_data(light_panel.point0, light_panel.point1, light_panel.normal);
        var intersect_data: PointIntersectData;
        ...
        let scatter_data = generate_scatter_ray_dir(hit_info);
...
fn shade_point_array(point_array: HitInfoArrayData) -> vec3<f32> {
    var info_array: array<PointIntersectData, 20> = point_array.data;
...
fn main([[builtin(global_invocation_id)]] gi_id: vec3<u32>) {
    ...
    loop {
        ...
        let hit_array_data = generate_hit_info_array(ray_origin, ray_direction);
    ...
    res_color.r = clamp(sqrt(out_color.x), 0.0, 1.0);
    res_color.g = clamp(sqrt(out_color.y), 0.0, 1.0);
    res_color.b = clamp(sqrt(out_color.z), 0.0, 1.0);
